...
class Hist1D(Node):
  ...
  def plot(self, fname, sname, burst_id, hist):
    ...
    rms = np.sqrt(hist.variance())
    skewness = hist.skewness() / np.power(rms, 3)
    median = hist.bin_edge(hist.median_bin())
    mode = hist.bin_edge(hist.mode_bin())
    if make_script:
      sfile = open(sname, 'w')
      ...
      sfile.write('# Script:    {}\n'.format(sname))
      sfile.write('# Image:     {}\n'.format(fname))
      sfile.write('# count = {}, mean = {}, rms = {}, skewness = {}, median = {}, mode = {}\n'.format(hist.count, hist.mean(), rms, skewness, median, mode))
    ...
    print('H Mode:      {}'.format(mode))
    print('H Median:    {}'.format(median))
    print('H Mean:      {}'.format(hist.mean()))
    print('H RMS:       {}'.format(rms))
    print('H Skewness:  {}'.format(skewness))
    print('=====================')
